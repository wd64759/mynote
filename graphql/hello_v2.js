// Function: build the inner link + branch level condition
// query {
//     job(typ: "Java") {
//       name,
//       typ,
...
const root = {    
    ...
    company({employer}) {
        ...
        const comps = [
            {name: 'Sony', industry: 'media', size: 10000, city_codes: ["001", "003"]},
            {name: 'IBM', industry: 'giant', size: 10000, city_codes: ["002", "003"]},
    ...
    job({typ, salary}) {
        ...
        const jobs = [
            ...
            {name: 'C11', typ: 'Web', status: 'Open', salary: 20000, openDays: 1, employer: 'Oracle', location: '002'},
            {name: 'C12', typ: 'Python', status: 'Open', salary: 26000, openDays: 2, employer: 'Oracle', location: '003'}
        ];
        const found = jobs.filter(e=>{ return e.typ === typ });
        if (found) {
            for (let job of found) {
                const com = this.company({employer: job.employer});
                job.company = com;
